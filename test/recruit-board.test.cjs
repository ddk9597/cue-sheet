...
const recruitRegionsMigration = fs.readFileSync(path.join(ROOT, "migrations/007_add_recruit_post_regions.sql"), "utf8");
...
test("모집 유형과 복수 지역은 주요 카테고리로 조합하고 상세 검색은 접어 둔다", () => {
  for (const region of [
    "서울",
    "경기",
    "인천",
    ...
    assert.match(html, new RegExp(`name="regionCategories" value="${region}"`));
  }

  assert.doesNotMatch(html, /name="region" type="text"/);
  assert.match(html, /<details id="recruitAdvancedSearch"/);
  assert.match(html, /type="checkbox" name="regionCategories"/);
  assert.match(client, /regions: new Set\(\)/);
  assert.match(client, /formData\.getAll\("regionCategories"\)/);
  assert.match(client, /post\.regionCategories\.some\(\(region\) => state\.regions\.has\(region\)\)/);
...
test("게시글 저장 스키마와 API 라우트가 등록되어 있다", () => {
  ...
  assert.match(recruitRegionsMigration, /ADD COLUMN IF NOT EXISTS region_categories TEXT\[\]/);
  assert.match(recruitRegionsMigration, /SET region_categories = ARRAY\[region_category\]/);
  ...
  assert.match(recruitRoute, /normalizeRequestedRegionSelection\(/);
  assert.match(schema, /region_category TEXT NOT NULL DEFAULT '전국·온라인'/);
  assert.match(schema, /region_categories TEXT\[\] NOT NULL DEFAULT ARRAY\[\]::TEXT\[\]/);
  assert.match(schema, /SET region_categories = ARRAY\[region_category\]/);
...
test("API는 여러 악기 파트와 지역을 하나의 게시글에 저장하고 반환한다", async () => {
  let insertParams = [];
  ...
  activeRecruitSql = {
    async query(statement, params = []) {
      ...
      if (query.startsWith("insert into recruit_posts")) {
        ...
        assert.match(query, /region_category, region_categories/);
        assert.match(query, /\$7::text\[\]/);
        return [{ id: 501 }];
      }
  ...
  assert.deepEqual(insertParams.slice(4, 7), ["서울, 경기", "서울", ["서울", "경기"]]);
  ...
  assert.deepEqual(response.body.post.regionCategories, ["서울", "경기"]);
  assert.equal(response.body.post.region, "서울, 경기");
});
